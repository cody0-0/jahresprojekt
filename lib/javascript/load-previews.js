...
clearButton.addEventListener('click', clearAllFilter);
...
function toggleFilter(key, value, override = false) {
    ...
    if (url.searchParams.has(key, value)) {
        ...
        if (noneActive) clearButton.classList.remove('active');
    } else {
        ...
        if (key === 'cos') {
            const other = value === 'minf' ? 'musk' : 'minf';
            url.searchParams.delete(key, other);
            const otherButtons = Array.from(buttons).filter(b => b.dataset.val !== value);
            otherButtons.forEach(b => {
                if (b.dataset.val && b.classList.contains('active')) {
...
function removeFilter(key) {
    ...
    const buttons = document.querySelectorAll(`button[data-param="${key}"]`);
    buttons.forEach(b => {
        if (b.classList.contains('active')) noneActive = false;
    });
    if (noneActive) clearButton.classList.remove('active');
...
function clearAllFilter() {
    ...
    url.searchParams.delete('cos');
    url.searchParams.delete('min-date');
    url.searchParams.delete('max-date');
    url.searchParams.set('preview-page', '0');
    // TODO: reset date input elements (wheel, slider)
...
function addDummyData(data) {
    ...
    for (let i = 0; i < dummyAmount; i++) {
        const item = {
            id: -i - 1,
            title: `[PLATZHALTER] Beitrags-Title${Math.random() > 0.5 ? ', gerne auch mal länger' : ''}`,
            title_image: -1,
