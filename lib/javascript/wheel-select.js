function selectWheelItem(wheelElem, fromEvent = null) {
    ...
    let selected = 0;
    if (fromEvent === "scrollend") selected = Math.round((wheelElem.scrollTop - _getHeightOfList(wheelElem.querySelectorAll('li')) / children.length / 2) / (_getHeightOfList(wheelElem.querySelectorAll('li')) / children.length));
    else {
