document.querySelectorAll('.date-filter-select').forEach((elem) => {
    ...
    if (elem.classList.contains('wheel-select')) {
        ...
        // scrollend event is not supported by all browsers
        let isScrolling;
    ...
    //'scrollend' fallback for browsers that don't support the scrollend event

});

function initFilter(elem, event) {
